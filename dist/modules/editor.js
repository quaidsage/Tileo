...
export function updateHTMLValues() {
    ...
    if (!(currentElement instanceof Empty)) {
        var elementName = inputElements.get('elementName');
        var elementButton = document.getElementById(currentElement.constructor.name.toLowerCase());
    ...
    var elementInstance = currentElement.constructor(0);
    inputElements.delete('elementName');
    inputElements.get('elementColor').value = '#' + elementConstructor.currentColor.map(function (val) { return Math.floor(val).toString(16).padStart(2, '0'); }).join('');
