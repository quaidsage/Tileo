...
let currentBrushMenu: HTMLDivElement | null;

function updatePreview() {
    // Get preview and content
    let previewCanvas = document.getElementById('brush-preview-canvas') as HTMLCanvasElement;
    let ctx = previewCanvas.getContext('2d') as CanvasRenderingContext2D;
...
function addControls() {
    ...
    currentBrushMenu?.appendChild(brushControls);
    ...
    brushSpeedSlider.oninput = (e) => {
        brushSpeed = parseInt((e.target as HTMLInputElement).value);
    ...
    brushControls?.appendChild(closeButton);
}
...
function addPreview() {
    ...
    currentBrushMenu?.appendChild(brushPreview);
...
export function setBrushSpeed(speed: number) {
    brushSpeed = speed;
}
