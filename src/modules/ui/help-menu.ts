...
let existingHelpMenu : HTMLDivElement | null = null;
...
function addContent() {
    // Add text to help menu based on the device type
    let helpText = document.createElement('p');
    if (isTouch()) { // Mobile
        helpText.innerHTML = `
            <h1>Mobile Help Text To Come... 👋</h1>
        `;
    } else {
        helpText.innerHTML = `
            <h1>Welcome 👋</h1>
    ...
    existingHelpMenu?.appendChild(helpText);
...
export function toggleHelpMenu(toggle?: boolean) {
    if (toggle) {
        focusCanvas();
        openHelpMenu()
